<h1 style="text-decoration: none; border-bottom: none;">클러스터를 활용한 멀티 프로세스 WebSocket 서버 아키텍처</h1>

## 1. cluster.ts의 기능 설명

`cluster.ts`는 마스터 프로세스로서 클러스터 전체를 관리하고 로드 밸런싱을 담당하는 파일입니다.
...
`server.ts`는 워커 프로세스에서 실행되는 WebSocket 서버를 초기화하고 관리하는 파일입니다.
